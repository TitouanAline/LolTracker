...
import { RiotService } from '../../core/services/riot';
...
export class SummonerComponent {
  ...
  result$ = this.searchTrigger.pipe(
    ...
    switchMap((params) => {
      ...
      return this.riotService.getAccount(params.name, params.tag).pipe(

        switchMap((account: any) =>
          this.riotService.getLastChampion(account.puuid)
        ),
  ...
  switchMap((friends) => {
    return Promise.all(
      friends.map(async (f): Promise<FriendResult> => {
        try {
          const account: any = await this.riotService.getAccount(f.name, f.tag).toPromise();
          const match: any = await this.riotService.getLastChampion(account.puuid).toPromise();
